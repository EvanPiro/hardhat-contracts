...
contract Forward {
    ...
    /**
     * @dev Creates an instance of `Hedge` where a price feeder is set for sourcing the price
     ...
     */
    constructor(address priceFeedAddress, uint unlockTime) payable {
        require(
     ...
     * is sent to the party that initiated the contract while the remaining ETH is send to the
     * counterparty.
     *
     * Note that the send of the remaining ETH to the counterparty is handled implicitly in the self
     * destruct function.
     */
    function unlock() external {
        ...
        if (_newPrice == 0) {
            Address.sendValue(_party, _contractBalance);
            ...
            if (_partyValue > _contractBalance) {
                ...
                Address.sendValue(_party, _partyValue);
            }
            emit ForwardUnlocked();
            selfdestruct(_counterparty);
        }
    }
}
